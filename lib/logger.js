...
class LoggerCreator {
	constructor(options) {
		...
		this.logger = new Logger(this.printer.emit, this.levels, {
			...
			secret: {
				value: (value, replacement = options.secretReplacement, level = options.secretLevel) => {
					const isHidden = this.levs.get(level) > this.printer.maxLevel;
					return new Secret(value, replacement, level, isHidden);
				},
				writable: false
			}
		});
