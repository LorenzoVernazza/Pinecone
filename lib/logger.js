...
class Lumberjack {
	constructor(levels, options = {}) {
		...
		if (level && this.levels.values[level]) {
			this.maxLevel = this.levels.values[level];
		} else if (level > -1 && level < this.levels.length) {
			this.maxLevel = level;
		} else {
			this.maxLevel = this.levels.maxLevel;
		}
...
class LoggerCreator {
	constructor(options) {
		...
		this.logger = new Logger(this.printer.emit, this.levels, {
			...
			secret: {
				value: (value, replacement = options.secretReplacement, level = options.secretLevel) => (new Secret(value, replacement, level)),
				writable: false
			}
		});
