...
function loadLevels(options){
    ...
    const getLogFunction = (value, level) => {
        ...
        return buildFunction(line, level);
    }
    if (noLog) {
        this.log = () => {};
    } else {
       this.log = (...value) => this.stdout.write(getLogFunction(value, {
            level: null,
            color: null,
            index: -1
        }));
...
function loadOptions(options){
    ...
    if (_opts.disableColors) _opts.colorObjects = false;
...
class Interface {
    constructor (options = {}){
        this.seed = 'seed-' + (idCounter++);
        this.apply = (options) => {
            const _opts = loadOptions(options);
            loadLevels.call(this, _opts);
            this.inspect = inspect.bind({
                colors: _opts.colorObjects,
                depth: _opts.depth
            });
