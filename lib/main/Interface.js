...
const { levels, defaults: defaultOpts } = require('../../config');
...
const Quotes = require('../classes/Quotes');
...
function loadLevels(options, emitter = '') {
	...
	levels.forEach(({ level, color, error }, index) => {
		...
		if (maxLevel >= requiredLevel && index <= maxLevel) {
			if (levelObj.error) {
...
function loadOptions(newOptions, oldOptions) {
	...
	const requiredLevel = levelsHandler.get(_opts.requiredLevel);
	_opts.requiredLevel = (requiredLevel === undefined) ? 0 : requiredLevel;
...
class Interface {
	constructor(options = {}) {
		...
		this.apply = (newOptions) => {
			...
			const { sawmill } = loadLevels.call(this, _opts);
			this.singleQuote = (...values) => (Quotes.quote(values, '\'', sawmill.process));
			this.doubleQuote = (...values) => (Quotes.quote(values, '"', sawmill.process));
			this.backTickQuote = (...values) => (Quotes.quote(values, '`', sawmill.process));
			...
			this.secret = (value, {
				...
				const _level = levelsHandler.get(level);
				const hidden = _level > _opts.level;
				if (iterable) {
					const secrets = new SecretArray(sawmill.process, level);
					if (Array.isArray(value)) {
						for (const _value of value) {
							secrets.add(_value, sawmill.process, {
								mask,
								level: _level,
								hidden,
								maxLength
						...
						secrets.add(value, sawmill.process, {
							mask,
							level: _level,
							hidden,
							maxLength
					...
					return new Secret(value, sawmill.process, {
						mask,
						level: _level,
						hidden,
						maxLength
			...
			for (const [emitterName, emitter] of Object.entries(_emitters)) {
				const { _requiredLevel } = emitter;
				loadLevels.call(emitter, _requiredLevel ? {
					..._opts,
		...
		this.emitter = (name = 'emitter', {
			...
			if (!_emitters[name]) {
				const _requiredLevel = requiredLevel && levelsHandler.get(requiredLevel);
				const emitter = {
					...(_requiredLevel && { _requiredLevel })
				};
				loadLevels.call(emitter, _requiredLevel && (_requiredLevel > (_opts.requiredLevel || 0)) ? {
					..._opts,
					requiredLevel: _requiredLevel
				} : _opts, name);
