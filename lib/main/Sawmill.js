...
class Sawmill {
	...
	process(value, level = {}) {
		var line = '';
		let space = '';
		const processOne = (element) => {
			if (element !== null && element !== undefined) {
				if (element instanceof NSP) {
					space = '';
				} else if (element instanceof Secret) { // this.levelsHandler.values[element.level]
					if (element.value instanceof NSP) {
						space = '';
					} else if (element.level.value > this.maxLevel) {
